...
#define HH_ENUM_INFO_TYPE                                                      \
  0x0007 // Get Info type name, call repeatedly to enumerate, -1 at end
...
#define HH_DISPLAY_TEXT_POPUP                                                  \
  0x000E // display string resource id or text in a popup window
#define HH_HELP_CONTEXT 0x000F // display mapped numeric value in dwData
#define HH_TP_HELP_CONTEXTMENU                                                 \
  0x0010 // text popup help, same as WinHelp HELP_CONTEXTMENU
#define HH_TP_HELP_WM_HELP                                                     \
  0x0011 // text popup help, same as WinHelp HELP_WM_HELP
#define HH_CLOSE_ALL                                                           \
  0x0012 // close all windows opened directly or indirectly by the caller
...
#define HH_ENUM_CATEGORY                                                       \
  0x0015 // Get category name, call repeatedly to enumerate, -1 at end
#define HH_ENUM_CATEGORY_IT                                                    \
  0x0016 // Get category info type members, call repeatedly to enumerate, -1 at
         // end
#define HH_RESET_IT_FILTER                                                     \
  0x0017 // Clear the info type filter of all info types.
#define HH_SET_INCLUSIVE_FILTER                                                \
  0x0018 // set inclusive filtering method for untyped topics to be included in
         // display
#define HH_SET_EXCLUSIVE_FILTER                                                \
  0x0019 // set exclusive filtering method for untyped topics to be excluded
         // from display
...
#define HH_SET_GLOBAL_PROPERTY                                                 \
  0x00fc // Set a global property. (NULL, NULL, HH_GPROP)

#define HHWIN_PROP_TAB_AUTOHIDESHOW                                            \
...
#define HHWIN_PROP_NODEF_STYLES                                                \
  (1 << 3) // no default window styles (only HH_WINTYPE.dwStyles)
#define HHWIN_PROP_NODEF_EXSTYLES                                              \
...
typedef struct tagHH_POPUP {
  ...
  UINT idString;   // string resource id, or text id if pszFile is specified in
                   // HtmlHelp call
  ...
  RECT rcMargins;  // amount of space between edges of window and text, -1 for
...
typedef struct tagHH_AKLINK {
  ...
  LPCTSTR pszMsgText; // Message text to display in MessageBox if pszUrl is NULL
                      // and no keyword match
  LPCTSTR pszMsgTitle; // Message text to display in MessageBox if pszUrl is
                       // NULL and no keyword match
...
typedef struct tagHH_ENUM_IT {
  ...
  int iType;    // the type of the information type ie. Inclusive, Exclusive, or
                // Hidden
  LPCSTR pszCatName; // Set to the name of the Category to enumerate the info
                     // types in a category; else NULL
  LPCSTR pszITName;  // volitile pointer to the name of the infotype. Allocated
                     // by call. Caller responsible for freeing
  LPCSTR
      pszITDescription; // volitile pointer to the description of the infotype.
...
typedef struct tagHH_SET_INFOTYPE {
  ...
  LPCSTR pszCatName;      // the name of the category, if any, the InfoType is a
                          // member of.
...
typedef struct tagHH_WINTYPE {
  ...
  int tabpos;        // IN/OUT: HHWIN_NAVTAB_TOP, HHWIN_NAVTAB_LEFT, or
                     // HHWIN_NAVTAB_BOTTOM
  int idNotify;      // IN: ID to use for WM_NOTIFY messages
  BYTE tabOrder[HH_MAX_TABS +
                1]; // IN/OUT: tab order: Contents, Index, Search, History,
                    // Favorites, Reserved 1-5, Custom tabs
...
typedef enum tagHH_GPROPID {
  ...
  HH_GPROPID_TOOLBAR_MARGIN =
      2, // long: Provides a left/right margin around the toolbar.
