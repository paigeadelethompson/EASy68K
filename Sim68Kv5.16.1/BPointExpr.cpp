...
int BPointExpr::getId() { return id; }

void BPointExpr::setId(int _id) { id = _id; }
...
AnsiString BPointExpr::getExprString() { return expr; }

void BPointExpr::setExprString(AnsiString _expr) { expr = _expr; }

int BPointExpr::getCount() { return count; }

void BPointExpr::setCount(int _count) { count = _count; }

bool BPointExpr::isEnabled() { return isEnab; }

void BPointExpr::isEnabled(bool _isEnab) { isEnab = _isEnab; }
...
bool BPointExpr::isBreak() {
  ...
  if (isBrk)
    bpCountCond[id] = 0; // Reset counter for next iteration.
  return isBrk;
}
