...
//---------------------------------------------------------------------------
__fastcall TBreaksFrm::TBreaksFrm(TComponent *Owner) : TForm(Owner) {
  ...
  RegValueMaskEdit->Left = curLeft +=
      RegStringGrid->ColWidths[REG_OPERATOR_COL];
  ...
  AddrSelectMaskEdit->Left = curLeft +=
      AddrStringGrid->ColWidths[ADDR_BRK_ID_COL];
  AddrOperatorCombo->Left = curLeft +=
      AddrStringGrid->ColWidths[ADDR_SELECT_COL];
  AddrValueMaskEdit->Left = curLeft +=
      AddrStringGrid->ColWidths[ADDR_OPERATOR_COL];
  AddrSizeCombo->Left = curLeft += AddrStringGrid->ColWidths[ADDR_VALUE_COL];
  AddrReadWriteCombo->Left = curLeft +=
      AddrStringGrid->ColWidths[ADDR_SIZE_COL];
  ...
  ExprEnabledCombo->Left = curLeft +=
      ExprStringGrid->ColWidths[EXPR_BRK_ID_COL];
...
void __fastcall TBreaksFrm::setExprButtons() {
  ...
  if (ExprEnabledCombo->Visible) {
    ...
    if (parenCount == 0 &&
        (mruOperand || (infixExpr[infixCount - 1] == RPAREN)))
  ...
  if (ExprExprEdit->Visible) {
    ...
    if (infixCount == 0 || mruOperator || (infixExpr[infixCount - 1] == LPAREN))
...
void __fastcall TBreaksFrm::RegSetButtonClick(TObject *Sender) {
  ...
  if ((RegSelectCombo->Text == "") || (RegOperatorCombo->Text == "") ||
      (RegValueMaskEdit->EditText == "") || (RegSizeCombo->Text == "")) {
    Application->MessageBox("Entry not stored.  Fields missing.", NULL, MB_OK);
    return;
  }
  ...
  RegOperatorCombo->Visible = false;
  RegStringGrid->Cells[REG_VALUE_COL][reg_edit_row] =
...
void __fastcall TBreaksFrm::AddrSetButtonClick(TObject *Sender) {
  // Make sure valid data was entered in each field
  if ((AddrSelectMaskEdit->EditText == "") || (AddrOperatorCombo->Text == "") ||
      (AddrValueMaskEdit->EditText == "") || (AddrReadWriteCombo->Text == "") ||
      (AddrSizeCombo->Text == "")) {
    Application->MessageBox("Entry not stored.  Fields missing.", NULL, MB_OK);
    return;
  }
  // Capture the text from editable fields into the AddrStringGrid.
  AddrStringGrid->Cells[ADDR_SELECT_COL][addr_edit_row] =
      AddrSelectMaskEdit->EditText;
  AddrSelectMaskEdit->Visible = false;
  AddrStringGrid->Cells[ADDR_OPERATOR_COL][addr_edit_row] =
      AddrOperatorCombo->Text;
  AddrOperatorCombo->Visible = false;
  AddrStringGrid->Cells[ADDR_VALUE_COL][addr_edit_row] =
      AddrValueMaskEdit->EditText;
  AddrValueMaskEdit->Visible = false;
  AddrStringGrid->Cells[ADDR_SIZE_COL][addr_edit_row] = AddrSizeCombo->Text;
  AddrSizeCombo->Visible = false;
  AddrStringGrid->Cells[ADDR_READ_WRITE_COL][addr_edit_row] =
...
void __fastcall TBreaksFrm::ExprSetButtonClick(TObject *Sender) {
  // Make sure valid data was entered in each field
  if ((ExprEnabledCombo->Text == "") || (ExprExprEdit->Text == "") ||
  ...
  ExprStringGrid->Cells[EXPR_ENABLED_COL][expr_edit_row] =
      ExprEnabledCombo->Text;
  ExprEnabledCombo->Visible = false;
  ExprStringGrid->Cells[EXPR_EXPR_COL][expr_edit_row] = ExprExprEdit->Text;
  ExprExprEdit->Visible = false;
  ExprStringGrid->Cells[EXPR_COUNT_COL][expr_edit_row] =
      ExprCountMaskEdit->EditText;
  ExprCountMaskEdit->Visible = false;
  ...
  AnsiString str = "0x";
  bpExpressions[expr_edit_row - 1].setCount(
      StrToInt(str + ExprCountMaskEdit->EditText));
  ...
  if (infixCount < MAX_LB_NODES) {
    ...
    if (regIndex > 0 && regIndex <= regCount) {
      ...
      Application->MessageBoxA(
          "Invalid PC/Reg Breakpoint. Set PC/Reg in Registers area first.",
          NULL, MB_OK);
    }
...
void __fastcall TBreaksFrm::ExprAddrAppendButtonClick(TObject *Sender) {
  ...
  if (infixCount < MAX_LB_NODES) {
    ...
    if (addrIndex > ADDR_ID_OFFSET && addrIndex <= ADDR_ID_OFFSET + addrCount) {
      ...
      Application->MessageBoxA(
          "Invalid Memory Breakpoint. Set Memory in Memory area first.", NULL,
          MB_OK);
    }
    mruOperand = true;
    mruOperator = false;
...
void __fastcall TBreaksFrm::ExprBackspaceButtonClick(TObject *Sender) {
  ...
  if (infixCount > 0 && infixExpr[infixCount - 1] < MAX_BPOINTS &&
      infixExpr[infixCount - 1] >= 0)
    mruOperand = true;
  else {
    mruOperand = false;
...
void __fastcall TBreaksFrm::RegClearButtonClick(TObject *Sender) {
  if (reg_edit_row > 0 && reg_edit_row <= regCount) {
    ...
    Application->MessageBoxA("Select a Valid Row.", NULL, MB_OK);
  }
  setRegButtons();
}
//---------------------------------------------------------------------------
...
void __fastcall TBreaksFrm::RegStringGridDblClick(TObject *Sender) {
  ...
  if (RegStringGrid->Row > regCount) {
    ...
    RegOperatorCombo->Text =
        RegStringGrid->Cells[REG_OPERATOR_COL][reg_edit_row];
...
void __fastcall TBreaksFrm::AddrStringGridDblClick(TObject *Sender) {
  ...
  if (AddrStringGrid->Row > addrCount) {
    ...
    // Set editable fields to existing values
    addr_edit_row = AddrStringGrid->Row;
    AddrSelectMaskEdit->Text =
        AddrStringGrid->Cells[ADDR_SELECT_COL][addr_edit_row];
    AddrOperatorCombo->Text =
        AddrStringGrid->Cells[ADDR_OPERATOR_COL][addr_edit_row];
    AddrValueMaskEdit->Text =
        AddrStringGrid->Cells[ADDR_VALUE_COL][addr_edit_row];
    ...
    AddrReadWriteCombo->ItemIndex =
        breakPoints[reg_edit_row - 1].getReadWrite();
  }
...
void __fastcall TBreaksFrm::ExprStringGridDblClick(TObject *Sender) {
  ...
  if (ExprStringGrid->Row > exprCount) {
    ...
    // Reset the expression array elements, so a new expression can be
    // constructed.
