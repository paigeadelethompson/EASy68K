...
//---------------------------------------------------------------------------
__fastcall TMemoryFrm::TMemoryFrm(TComponent *Owner) : TForm(Owner) {}
//---------------------------------------------------------------------------
...
void __fastcall TMemoryFrm::LivePaint(unsigned int loc) {
  if (loc >= memAddr - 3 && loc <= (memAddr + nRows * 16) && loc < MEMSIZE &&
...
//---------------------------------------------------------------------------
void __fastcall TMemoryFrm::FormResize(TObject *Sender) { Repaint(); }
//---------------------------------------------------------------------------
...
void __fastcall TMemoryFrm::FormMouseWheelUp(TObject *Sender, TShiftState Shift,
...
//---------------------------------------------------------------------------
// Mouse Button Down
void __fastcall TMemoryFrm::FormMouseDown(TObject *Sender, TMouseButton Button,
                                          TShiftState Shift, int X, int Y) {
  erasePrompt(); // erase old prompt
  Address1
      ->SetFocus(); // give address box focus to remove focus from everyone else
...
void __fastcall TMemoryFrm::promptTimer(TObject *Sender) { drawPrompt(); }
...
void __fastcall TMemoryFrm::CopyClick(TObject *Sender) {
  try {
    ...
    if (fromAddr < toAddr) {
      ...
      for (i = 0; i < bytes; i++) {
        if (toAddr + i >= MEMSIZE ||
            fromAddr + i >= MEMSIZE) { // if invalid address
          ShowMessage("Invalid copy range");
          return;
        }
        memory[toAddr + i] = memory[fromAddr + i]; // copy a byte
...
void __fastcall TMemoryFrm::SaveClick(TObject *Sender) {
  ...
  try {
    if (SaveDialog->Execute()) {
      ...
      if (FileExists(str)) {
        int response = Application->MessageBox("File exists! OK to replace?",
                                               "Alert", MB_YESNO);
        if (response == IDNO)
          return;
      }
      ofstream oFile(str.c_str(), ios::binary);
      oFile.write(reinterpret_cast<char *>(&memory[fromAddr & ADDRMASK]),
                  bytes);
      oFile.close();
    }
  } catch (...) {
    MessageDlg("Error saving 68K memory to file.", mtInformation,
