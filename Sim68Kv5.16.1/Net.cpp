...
int __fastcall netCreateClient(char *server, int port, int protocol) {
  ...
  int status;
  int timeout =
      5000; // attempt to connect for 5 seconds before returnning an error
  ...
  if ((remote.sin_addr.s_addr = inet_addr(server)) == INADDR_NONE) {
    ...
    sprintf(serverIP, "%d.%d.%d.%d", (unsigned char)host->h_addr_list[0][0],
...
int __fastcall netReadData(char *data, unsigned int &size, char *senderIP) {
  ...
  if (sock != NULL) {
    ...
    ret = recvfrom(sock, data, readSize, 0, (SOCKADDR *)&remote,
                   (int *)&remoteAddrSize);
    if (ret == SOCKET_ERROR) {
      status = WSAGetLastError();
      if (status != WSAEWOULDBLOCK) // don't report WOULDBLOCK error
...
int __fastcall netLocalIP(char *localIP) {
  ...
  sprintf(localIP, "%d.%d.%d.%d", (unsigned char)host->h_addr_list[0][0],
          (unsigned char)host->h_addr_list[0][1],
          (unsigned char)host->h_addr_list[0][2],
          (unsigned char)host->h_addr_list[0][3]);
  return NET_OK;
