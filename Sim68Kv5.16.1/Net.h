...
int __fastcall netCreateServer(int port,
                               int protocol); // setup network for server
int __fastcall netCreateClient(char *server, int port,
                               int protocol); // setup network for client
int __fastcall netLocalIP(char *localIP);     // returns localIP
int __fastcall netSendData(char *data, unsigned int &size,
                           char *remoteIP); // sends network data
int __fastcall netReadData(char *data, unsigned int &size,
                           char *senderIP); // returns network data
int __fastcall netSendData(char *data, unsigned int &size, char *remoteIP,
                           USHORT port); // sends network data
int __fastcall netReadData(char *data, unsigned int &size, char *senderIP,
                           USHORT &port); // returns network data
