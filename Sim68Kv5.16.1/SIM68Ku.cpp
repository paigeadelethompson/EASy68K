...
HTML_HELP_PROC
    __HtmlHelp; // function pointer for the HtmlHelp() function. Note
...
__fastcall TForm1::TForm1(TComponent *Owner) : TForm(Owner) {
  ...
  if (!LoadHtmlHelp()) {
    ShowMessage("HTML Help was not detected on this computer. EASy68K help may "
                "not work correctly.");
  }
  ...
  try {
    ...
    Application->MessageBox(
        "Error reserving main 68000 memory in Form1::FormCreate", NULL, mbOK);
...
__fastcall TForm1::~TForm1() {
  try {
    ...
    /*
    WARNING: When you close an application that uses HtmlHelp and at the point
    of exit may have HTML Help Windows still open, you _must_ close these
    windows prior to exit. Failing to do this may result in access violations.
    ...
    There are a few gotcha's when using the HH_CLOSE_ALL command with a
    dynamically loaded HTML Help API. More specifically the problem lies in
    unloading the HTML Help API. You want to be a good Windows citizen so you
    unload all dynamically loaded librarys before you terminate. So after the
    mandatory HH_CLOSE_ALL you call FreeLibrary and then...bang, an access
    violation. The problem is that normally HH_CLOSE_ALL results in the creation
    of a secondary thread that performs the actual action of closing all open
    HTML Help Windows while your application thread is allowed to continue
    running. If you immediately call FreeLibrary after HH_CLOSE_ALL, that
    secondary - which exists in hhctrl.ocx, may still be running. When it
    continues running the operating system finds that hhctrl.ocx is already
    unloaded and an access violation occurs. There are multiple ways to work
    around this problem but the recommended one is to use HH_INITIALIZE when you
    load HTML HELP and HH_UNINITIALIZE before you unload HTML Help, as is done
    in this code (doing this results in HH_CLOSE_ALL not spawning a secondary
    thread but using the calling thread, eliminating the problem).
    */
    if (HHLibrary != 0) {
  ...
  try {
    ...
    if (loadSrec(name.c_str()) == false) { // if S-Record loads with no errors
      ...
      if (FileExists(str)) {
        ...
        //
        // *[sim68k]break      sets a PC breakpoint on the following line of
        // code:
        ...
        for (int i = 0; i < ListBox1->Items->Count; i++) {
          str =
              ListBox1->Items->Strings[i].LowerCase(); // force str to lowercase
          if (str.SubString(41, 14) == "*[sim68k]break") { // if break set
            ...
            {
              addr =
                  StrToInt("0x" + str.SubString(1, 8)); // get address from line
              BreaksFrm->sbpoint(addr);                 // set break point
            }
          }
          if (str.SubString(41, 17) ==
              "*[sim68k]bitfield") { // if bitfield enabled
            bitfield = true;
            BitFieldEnabled->Checked = true;
          }
          if (str.SubString(41, 20) ==
              "*[sim68k]simhalt_off") { // if turn off SIMHALT
        ...
        ListBox1->Items->Add(
            "A matching .L68 file was not found. The .L68 file");
...
void __fastcall TForm1::OpenDataExecute(TObject *Sender) {
  try {
    ...
    Form1->Message->Lines->Add(
        errstr.sprintf("Unexpected error in OpenDataExecute"));
    return;
  }
}
...
void __fastcall TForm1::WmDropFiles(TWMDropFiles &Message) {
  ...
  if (result == IDOK) {
    ...
    // int numFiles = DragQueryFile(hDrop, -1, NULL, NULL);  // number of files
    // dropped for (int i=0;i < numFiles;i++) {      // loop for all files
    // dropped DragQueryFile(hDrop, i, buff, sizeof(buff));        // get name
    // of file i
...
void __fastcall TForm1::RunToCursorExecute(TObject *Sender) {
  ...
  try {
    ...
    if (ListBox1->ItemIndex > 2 && isInstruction(str)) {
      ...
      Application->MessageBox("Selected line does not have a valid address.",
                              NULL, mbOK);
  } catch (...) {
...
void __fastcall TForm1::AboutExecute(TObject *Sender) { AboutFrm->ShowModal(); }
...
void __fastcall TForm1::breakPPaint(TObject *Sender) {
  ...
  for (Index = ListBox1->TopIndex; Index <= botIndex; Index++) {
    ...
    if (Index > 2 && isInstruction(str)) // if instruction
    {
      ...
      // erase old dot
      breakP->Canvas->FillRect(
          TRect(1, top, breakP->Width, top + ListBox1->ItemHeight));
    }
  }
}
...
void __fastcall TForm1::breakPMouseDown(TObject *Sender, TMouseButton Button,
...
void __fastcall TForm1::highlight() {
  ...
  // ListBox1->ItemIndex = -1;   // remove highlight *ck 12-3-2005 for better
  // macro behavior

  for (i = 3; i < ListBox1->Items->Count; i++) {
    str = ListBox1->Items->Strings[i];
    if (regPC->EditText ==
        str.UpperCase().SubString(1, 8)) // if current instruction
...
void __fastcall TForm1::FontOutputExecute(TObject *Sender) {
  if (FontDialogSimIO->Execute()) { // if OK selected
    simIO->BackBuffer->Canvas->Font->Assign(
        FontDialogSimIO->Font); // apply new font
...
void __fastcall TForm1::Stack1Click(TObject *Sender) { StackFrm->Show(); }
//---------------------------------------------------------------------------
...
void __fastcall TForm1::Memory1Click(TObject *Sender) { MemoryFrm->Show(); }
//---------------------------------------------------------------------------
...
void __fastcall TForm1::FormResize(TObject *Sender) { Form1->Repaint(); }
//---------------------------------------------------------------------------
...
void __fastcall TForm1::LogOutput1Click(TObject *Sender) { Log->Show(); }
//---------------------------------------------------------------------------
...
void __fastcall TForm1::displayHelp(char *context) {
  ...
  if (HHLibrary != 0)
    m_hWindow = __HtmlHelp(H, m_asHelpFile.c_str(), HH_HELP_CONTEXT,
                           getHelpContext(context));
  else
    ShowMessage("HTML Help was not detected on this computer. The HTML help "
                "viewer may be downloaded from msdn.microsoft.com");
}
...
void __fastcall TForm1::BringToFront() { Form1->SetFocus(); }
//---------------------------------------------------------------------------
...
void __fastcall TForm1::Hardware1Click(TObject *Sender) { Hardware->Show(); }
//---------------------------------------------------------------------------
...
void __fastcall TForm1::mmuFullscreenOptionsClick(TObject *Sender) {
  if (MultimonitorAPIsExist) {
    if (simIO->fullScreen == false) {
      ...
      Application->MessageBox("Please deactivate fullscreen mode before "
                              "attempting to change the fullscreen settings.",
                              NULL, MB_OK);
    }
  } else {
    Application->MessageBox(
        "Sorry, but your system does not seem to support multiple monitors.",
        NULL, MB_OK);
  }
}
...
void TForm1::OnDisplayChange(TWMDisplayChange &temp) {
  ...
  // Application->MessageBox(static_cast<AnsiString>(Form1->Left).c_str(), NULL,
  // MB_OK);
}
...
void __fastcall TForm1::LogStartExecute(TObject *Sender) { Log->startLog(); }
//---------------------------------------------------------------------------

void __fastcall TForm1::LogStopExecute(TObject *Sender) { Log->stopLog(); }
...
// saves simulator settings to file
{
  try {
    ...
    File << "$Settings for " << TITLE << "  DO NOT EDIT THIS FILE!!!!\n"
         ...
         << Hardware->seg7addr->EditText.Trim().c_str()
         << "$7-segment address\n"
...
void __fastcall TForm1::defaultSettings() {
  try {
    ...
    MessageDlg("Error setting default Sim68K settings", mtInformation,
               TMsgDlgButtons() << mbOK, 0);
    return;
  }
}
  ...
  try {
    ...
    {                         // if it did then load all the settings
      ...
      if (!strcmp(&buffer[index],
                  "$7-segment address")) { // if expected setting
      ...
      File.getline(buffer,
                   SIZE); // this is 'Hardware push button Address' setting
      index = 0;          // reset looping index
      while (buffer[index] != '$' &&
             index < strlen(buffer)) // we need to extract only the number
      {                              // which is all the chars before the '$'
      ...
      temp[index] = '\0';
      if (!strcmp(&buffer[index],
                  "$listing font name")) { // if expected setting
      ...
      temp[index] = '\0';
      if (!strcmp(&buffer[index],
                  "$listing font size")) { // if expected setting
      ...
      temp[index] = '\0';
      if (!strcmp(&buffer[index],
                  "$printer font name")) { // if expected setting
      ...
      temp[index] = '\0';
      if (!strcmp(&buffer[index],
                  "$printer font size")) { // if expected setting
      ...
      temp[index] = '\0';
      if (!strcmp(&buffer[index],
                  "$fullscreen monitor")) { // if expected setting
      ...
      temp[index] = '\0';
      if (!strcmp(&buffer[index],
                  "$auto trace interval")) { // if expected setting
    ...
    MessageDlg("Error loading Sim68k settings", mtInformation,
               TMsgDlgButtons() << mbOK, 0);
    return;
  }
}
...
void __fastcall TForm1::GotoPC1Click(TObject *Sender) { highlight(); }
//---------------------------------------------------------------------------
