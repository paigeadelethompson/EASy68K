...
//---------------------------------------------------------------------------
__fastcall TStackFrm::TStackFrm(TComponent *Owner) : TForm(Owner) {}
//---------------------------------------------------------------------------
...
void __fastcall TStackFrm::FormPaint(TObject *Sender) { DisplayStack(); }
//---------------------------------------------------------------------------

void __fastcall TStackFrm::FormResize(TObject *Sender) { DisplayStack(); }
...
void __fastcall TStackFrm::DisplayStack() {
  ...
  // erase bottom row
  TRect BotRect =
      Rect(0, ClientHeight - stackRowHeight, ClientWidth, ClientHeight);
  ...
  for (int r = 0; r < nRows; r++) {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (InvalidMap && (dispAddr + i >= InvalidStart) &&
          (dispAddr + i <= InvalidEnd))
      ...
      Canvas->TextOutA(
          scrPos.x, scrPos.y,
          (str.sprintf("%02hX ",
                       (unsigned char)memory[dispAddr + i & ADDRMASK])));
      scrPos = Canvas->PenPos;
...
void __fastcall TStackFrm::FormShow(TObject *Sender) { updateDisplay(); }
//---------------------------------------------------------------------------
...
void __fastcall TStackFrm::FormMouseWheelUp(TObject *Sender, TShiftState Shift,
                                            TPoint &MousePos, bool &Handled) {
  midAddr -= 4; // display lower address
  DisplayStack();
}
