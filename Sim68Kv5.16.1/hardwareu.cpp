...
//---------------------------------------------------------------------------
__fastcall THardware::THardware(TComponent *Owner) : TForm(Owner) {}
...
void __fastcall THardware::updateIfNeeded(int adr) {
  if (adr < MEMSIZE) { // if in valid memory space
    if ((adr >= seg7loc - 4 &&
         adr <= seg7loc + 15) ||                // if address in seg7 display
...
void __fastcall THardware::addrKeyUp(TObject *Sender, WORD &Key,
  ...
  if ((seg7loc >= MEMSIZE - 8) || // if invalid address
      (seg7loc <= LEDloc &&
       seg7loc + 15 >= LEDloc) || // if conflict with LED address
      (seg7loc <= switchLoc &&
       seg7loc + 15 >= switchLoc) || // if conflict with switch address
      (seg7loc <= pbLoc &&
       seg7loc + 15 >= pbLoc))     // if conflict with push button address
  ...
  if ((LEDloc >= MEMSIZE) || // if invalid address
      (LEDloc >= seg7loc &&
       LEDloc <= seg7loc + 15) || // if conflict with seg7 address
  ...
  if ((switchLoc >= MEMSIZE) || // if invalid address
      (switchLoc >= seg7loc &&
       switchLoc <= seg7loc + 15) || // if conflict with seg7 address
  ...
  if ((pbLoc >= MEMSIZE) || // if invalid address
      (pbLoc <= seg7loc + 15 &&
       pbLoc >= seg7loc) ||      // if conflict with seg7 address
...
void __fastcall THardware::IRQ1(TObject *Sender) { IRQprocess(1); }
//---------------------------------------------------------------------------

void __fastcall THardware::IRQ2(TObject *Sender) { IRQprocess(2); }
//---------------------------------------------------------------------------

void __fastcall THardware::IRQ3(TObject *Sender) { IRQprocess(3); }
//---------------------------------------------------------------------------

void __fastcall THardware::IRQ4(TObject *Sender) { IRQprocess(4); }
//---------------------------------------------------------------------------

void __fastcall THardware::IRQ5(TObject *Sender) { IRQprocess(5); }
//---------------------------------------------------------------------------

void __fastcall THardware::IRQ6(TObject *Sender) { IRQprocess(6); }
//---------------------------------------------------------------------------

void __fastcall THardware::IRQ7(TObject *Sender) { IRQprocess(7); }
//---------------------------------------------------------------------------
...
void __fastcall THardware::pb0MouseDown(TObject *Sender, TMouseButton Button,
...
void __fastcall THardware::pb1MouseDown(TObject *Sender, TMouseButton Button,
                                        TShiftState Shift, int X, int Y) {
  memory[pbLoc] &= 0xFD;
  update();
  MemoryFrm->Repaint();
...
void __fastcall THardware::pb2MouseDown(TObject *Sender, TMouseButton Button,
                                        TShiftState Shift, int X, int Y) {
  memory[pbLoc] &= 0xFB;
  update();
  MemoryFrm->Repaint();
...
void __fastcall THardware::pb3MouseDown(TObject *Sender, TMouseButton Button,
                                        TShiftState Shift, int X, int Y) {
  memory[pbLoc] &= 0xF7;
  update();
  MemoryFrm->Repaint();
...
void __fastcall THardware::pb4MouseDown(TObject *Sender, TMouseButton Button,
                                        TShiftState Shift, int X, int Y) {
  memory[pbLoc] &= 0xEF;
  update();
  MemoryFrm->Repaint();
...
void __fastcall THardware::pb5MouseDown(TObject *Sender, TMouseButton Button,
                                        TShiftState Shift, int X, int Y) {
  memory[pbLoc] &= 0xDF;
  update();
  MemoryFrm->Repaint();
...
void __fastcall THardware::pb6MouseDown(TObject *Sender, TMouseButton Button,
                                        TShiftState Shift, int X, int Y) {
  memory[pbLoc] &= 0xBF;
  update();
  MemoryFrm->Repaint();
...
void __fastcall THardware::pb7MouseDown(TObject *Sender, TMouseButton Button,
                                        TShiftState Shift, int X, int Y) {
  memory[pbLoc] &= 0x7F;
  update();
  MemoryFrm->Repaint();
