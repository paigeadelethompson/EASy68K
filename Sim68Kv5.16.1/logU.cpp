...
//---------------------------------------------------------------------------
__fastcall TLog::TLog(TComponent *Owner) : TForm(Owner) {}
  ...
  try {
    ...
    if (ElogFlag) // if execution log selected
    {
      ...
      if (ElogFile) {                                    // if file exists
        ...
        case mrAll:
          ElogFile =
              fopen(ELogFileName->Text.c_str(), "at"); // append to log file
          break;
        default:
          stopLog();
          return;
      ...
      if (!ElogFile) { // if file error
        sprintf(buffer,
                "Can't open log file. Check log file name., Error number %d\n",
                ElogFile);
    ...
    if (OlogFlag) // if output log selected
    {
      ...
      if (OlogFile) {                                    // if file exists
        ...
        case mrAll:
          OlogFile =
              fopen(OLogFileName->Text.c_str(), "ab"); // append to log file
          break;
        default:
          stopLog();
          return;
    ...
    sprintf(
        buffer,
        "ERROR 999: An exception occurred in routine 'TLog::OKBtnClick'. \n");
    Application->MessageBox(buffer, "Error", MB_OK);
  }
}
...
void __fastcall TLog::ELogTypeClick(TObject *Sender) {
  if (ELogType->ItemIndex ==
      INST_REG_MEM) { // if "Instruction, Registers and Memory"
    MemRange->Show();
  } else {
    MemRange->Hide();
  }
