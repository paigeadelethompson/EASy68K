...
void __fastcall TsimIO::setupWindow() {
  try {
    ...
    if (fullScreen) {
      ...
      // EnumDisplaySettings returns 0 for the frequency value indicating
      // "Default". However the default for different resolutions is different.
      // When you switch mode to a different frequency, you should set the
      // frequency variable to a different value from the current one if
      // EnumDisplaySettings indicated a different value was required. If it
      // indicated 0, meaning default, you should set the frequency as 0 but
      // also set the DM_FREQUENCY flag in the dmFields member of DEVMODE.  This
      // tells Windows that the new default (0) is not the same as the old
      // default (also 0 but potentially meaning something different from the
      // original 0).
      ...
      dmScSettings.dmFields = DM_BITSPERPEL | DM_PELSWIDTH | DM_PELSHEIGHT |
                              DM_DISPLAYFREQUENCY; // set the fields we used
      ...
      if (MultimonitorAPIsExist) {
        ...
        if ((*EnumDisplayDevicesAPtr)(NULL, FullScreenMonitor, &mydevice,
                                      NULL)) {
          strcpy(FullScreenDeviceName, mydevice.DeviceName);

          // if fullscreen change fails
          if ((*ChangeDisplaySettingsExAPtr)(FullScreenDeviceName,
                                             &dmScSettings, NULL,
            ...
            if (FullScreenMonitor != 0) {
              ...
              Application->MessageBoxA(
                  "Sorry, this computer's configuration does not support the "
                  "desired screen resolution in full screen mode.",
                  "Information");
              fullScreen = false;
              setupWindow();
              return;
          ...
          if (FullScreenMonitor != 0) {
            ...
            Application->MessageBoxA(
                "Sorry, the program encountered an error when trying to read "
                "your computer's monitor configuration.",
                "Information");
            fullScreen = false;
            return;
          }
        ...
        if ((*EnumDisplaySettingsExAPtr)(FullScreenDeviceName,
                                         ENUM_CURRENT_SETTINGS, &mydevmode,
                                         NULL) == false) {
          if (FullScreenMonitor != 0) {
            fullScreen = false;
            setupWindow();
        ...
        // if fullscreen change fails
        if (ChangeDisplaySettings(&dmScSettings, CDS_FULLSCREEN) !=
            DISP_CHANGE_SUCCESSFUL) {
          Application->MessageBoxA(
              "Sorry, this computer's configuration does not support the "
              "desired resolution in full screen mode.",
              "Information");
          fullScreen = false;
          return;
        }
      ...
      // if we can use the 98 and up APIs
      if (MultimonitorAPIsExist) {
        (*ChangeDisplaySettingsExAPtr)(FullScreenDeviceName, NULL, NULL, 0,
                                       NULL);
...
//-----------------------------------------------------------------------
// Display File Dialog
void __fastcall TsimIO::displayFileDialog(long *mode, int A1, int A2, int A3,
                                          short *result) {
  try {
    ...
    clearKeys(); // key up/down array is cleared because dialog intercepts key
                 // presses

    *result = F_SUCCESS; // default to success
    if (A1 != 0) {
      inTitle = &memory[A1 & ADDRMASK];
      strncpy(title, inTitle,
              255); // make copy of string so we can terminate it
    } else
      title[0] = '\0';
...
//---------------------------------------------------------------------------
void __fastcall TsimIO::FormActivate(TObject *Sender) { clearKeys(); }
...
//---------------------------------------------------------------------------
void __fastcall TsimIO::FormMouseMove(TObject *Sender, TShiftState Shift, int X,
                                      int Y) {
  try {
    mouseX = X;
    mouseY = Y;
...
void __fastcall TsimIO::textOut(AnsiString str) {
  try {
    ...
    for (int i = 1; i <= n; i++)
      charOut(str[i]);
    if (doubleBuffer == false)
      this->Repaint(); // ck, 2 textOut in a row resulted in delay before 2nd
                       // output was visible
  } catch (...) {
...
//---------------------------------------------------------------------------
void __fastcall TsimIO::setTextWrap(bool b) { textWrap = b; }
...
//---------------------------------------------------------------------------
// scrollRect
// scroll the text in the specified text rectangle up,down,left or right by one
// character dir = 0 up, 1 down, other values reserved invalid text areas are
// ignored
void __fastcall TsimIO::scrollRect(ushort r, ushort c, ushort h, ushort w,
                                   ushort dir) {
  try {
    TRect TopRect, BotRect, LeftRect, RightRect;

    if ((r + h >= MAX_ROWS) || (c + w >= MAX_COLS) || (w == 0) ||
        (h == 0)) // if invalid text rectangle
      return;
    ...
    case 0: // ^^^^^ UP ^^^^^
      ...
      // erase bottom row
      BotRect = Rect(c * colWidth, (r + h - 1) * rowHeight, (c + w) * colWidth,
                     (r + h) * rowHeight);
      BackBuffer->Canvas->CopyMode = cmBlackness; // fill canvas with black
      BackBuffer->Canvas->CopyRect(BotRect, BackBuffer->Canvas, BotRect);
      BackBuffer->Canvas->CopyMode = cmSrcCopy;
    ...
    case 1: // vvvvv DOWN vvvvv
      ...
      // erase top row
      TopRect = Rect(c * colWidth, r * rowHeight, (c + w) * colWidth,
                     (r + 1) * rowHeight);
      BackBuffer->Canvas->CopyMode = cmBlackness; // fill canvas with black
    ...
    case 2: // <<<<< LEFT <<<<<
      ...
      // erase right col
      RightRect = Rect((c + w - 1) * colWidth, r * rowHeight,
                       (c + w) * colWidth, (r + h) * rowHeight);
      BackBuffer->Canvas->CopyMode = cmBlackness; // fill canvas with black
    ...
    case 3: // >>>>> RIGHT >>>>>
      ...
      // erase left col
      LeftRect = Rect(c * colWidth, r * rowHeight, (c + 1) * colWidth,
                      (r + h) * rowHeight);
      BackBuffer->Canvas->CopyMode = cmBlackness; // fill canvas with black
      ...
      if (Key == '\r' || keyI >= 79 || charInput) {
        ...
        if (charInput) {
          *inputCh = Key; // get key
          ...
          {
            for (int i = 0; i <= keyI;
                 i++) // put string in 68000 memory at userBuf
        ...
        if (keyI < 0)
          ...
          if (keyboardEcho) {
            BackBuffer->Canvas->TextOutA(textX, textY,
                                         " "); // erase visible char
            if (doubleBuffer == false)
        ...
        if (keyboardEcho) {
          BackBuffer->Canvas->TextOutA(textX, textY,
                                       keyBuf[keyI - 1]); // display input char
          if (doubleBuffer == false)
            Canvas->TextOutA(textX, textY,
                             keyBuf[keyI - 1]);  // display input char
          if (logging && OlogFlag == TEXTONLY) { // if logging output
...
// post: result = true if sound played
//       result = 0 if player was busy playing a previous sound
void __fastcall TsimIO::controlSound(int control, int waveIndex,
                                     short *result) {
  try {
    *result = 0;                      // default to error condition
    if (wavemem[waveIndex] != NULL) { // if valid wave index
      ...
      case 1:                    // start looping sound
        currentWave = waveIndex; // save wave index
        *result =
            (short)PlaySound(wavemem[waveIndex], NULL,
...
//   pre: fileName is null terminated path address. Invalid file names are
//   ignored.
...
void __fastcall TsimIO::loadSoundDX(char *fileName, int waveIndex,
                                    short *result) {
  try {
    *result = 0;      // default to DirectX not available
    if (!dsoundExist) // if DirectSound not available
...
//   post: result = non zero on success
//         result = 0 on error
void __fastcall TsimIO::controlSoundDX(int control, int waveIndex,
                                       short *result) {
  try {
...
void __fastcall TsimIO::line(int x1, int y1, int x2, int y2) {
  ...
  BackBuffer->Canvas->Pixels[x2][y2] =
      BackBuffer->Canvas->Pen->Color; // plot to fix builder bug
  if (doubleBuffer == false) {
    Canvas->MoveTo(x1, y1);
    Canvas->LineTo(x2, y2);
...
void __fastcall TsimIO::rectangle(int x1, int y1, int x2, int y2) {
  ...
  BackBuffer->Canvas->Rectangle(x1, y1, x2 + 1,
                                y2 + 1); // +1 to fix builder bug
  if (doubleBuffer == false)
...
void __fastcall TsimIO::floodFill(int x, int y) {
  BackBuffer->Canvas->FloodFill(x, y, BackBuffer->Canvas->Pixels[x][y],
                                fsSurface);
  if (doubleBuffer == false)
...
void __fastcall TsimIO::setDrawingMode(int m) {
  ...
  if (m == 16) { // if turn off double buffering
    doubleBuffer = false;
    Canvas->PenPos =
        BackBuffer->Canvas->PenPos; // get x,y from BackBuffer->Canvas
...
void __fastcall TsimIO::initComm(int cid, char *portName, short *result) {
  *result = 0; // 0 is success
  ...
  // Open the Comm port.
  hComm[cid] = CreateFile(portName, GENERIC_READ | GENERIC_WRITE, 0, 0,
                          OPEN_EXISTING, 0, 0);
...
void __fastcall TsimIO::setCommParams(int cid, int settings, short *result) {
  ...
  *result = 0; // 0 is success
  ...
  case 2:
    ...
    break;
  default: // 5 data bits not supported because 5 data bits with 2 stop bits is
...
void __fastcall TsimIO::readComm(int cid, uchar *n, char *str, short *result) {
  ...
  *n = dwBytesRead;
  ...
  if (memoryMapCheck(Invalid | Read | Rom, (str - memory), (int)dwBytesRead) ==
      SUCCESS)
...
//---------------------------------------------------------------------------
// Create Client
void __fastcall TsimIO::createNetClient(int settings, char *server,
                                        int *result) {
  uint port, type;
...
//---------------------------------------------------------------------------
// Send
void __fastcall TsimIO::sendNet(int settings, char *data, char *remoteIP,
...
//---------------------------------------------------------------------------
// Receive
void __fastcall TsimIO::receiveNet(int settings, char *buffer, int *count,
                                   char *senderIP, int *result) {
  unsigned int size;
...
//---------------------------------------------------------------------------
// Send on port
void __fastcall TsimIO::sendPortNet(long *D0, long *D1, char *data,
                                    char *remoteIP) {
  unsigned int size;
...
void __fastcall TsimIO::receivePortNet(long *D0, long *D1, char *buffer,
                                       char *senderIP) {
  unsigned int size;
  unsigned short portNum;
